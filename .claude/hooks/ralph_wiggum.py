...
# Correct vault path for this project
PROJECT_ROOT = Path("/mnt/d/F drive backup/coding Q4/hackathon-0/ai-employee")
...
def main():
    # Emergency exit — touch stop_ralph file to bypass hook
    if STOP_FILE.exists():
        STOP_FILE.unlink()
        sys.exit(0)
    ...
    if pending:
        ...
        print(json.dumps({
            "decision": "block",
            "reason": (
                f"STOP BLOCKED by Ralph Wiggum Hook\n"
                f"There {'is' if len(pending) == 1 else 'are'} still {len(pending)} "
                f"unprocessed file{'s' if len(pending) != 1 else ''} in Needs_Action/:\n"
                f"{file_list}\n\n"
                f"Process each file using /process-file skill, "
                f"move completed tasks to Done/. "
                f"Do not stop until Needs_Action/ is empty.\n\n"
                f"To bypass: Create 'stop_ralph' file in vault."
            )
        }))
