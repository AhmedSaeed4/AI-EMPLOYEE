#!/usr/bin/env python3
"""
Weekly Audit Cron Trigger - Scheduled CEO Briefing Generation

This script is designed to be run via cron to automatically trigger
the weekly-audit skill for CEO briefings.
...
Flow:
  Cron → This Script → Claude Code → /weekly-audit skill
                            ...
                            Creates briefing in Briefings/
                                    ↓
                            Emails briefing to you
                                    ↓
                            Updates Dashboard with link

Usage:
    python weekly_audit_cron_trigger.py

Cron entry (Monday 6am):
    0 6 * * 1 cd /path/to/ai_employee_scripts && python scripts/weekly_audit_cron_trigger.py
"""
...
def setup_logging():
    ...
    # Create logger
    logger = logging.getLogger('WeeklyAuditCron')
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("WEEKLY AUDIT CRON TRIGGER STARTED")
    logger.info("=" * 60)
    ...
    logger.info("This will generate your CEO Briefing and email it to you.")
    logger.info("")
    ...
    if success:
        ...
        logger.info("")
        logger.info("Your CEO Briefing has been:")
        logger.info("  1. Generated and saved to Briefings/ folder")
        logger.info("  2. Emailed to your Gmail inbox")
        logger.info("  3. Linked in your Dashboard.md")
        logger.info("")
        logger.info("Have a productive week!")
        logger.info("=" * 60)
        sys.exit(0)
    else:
